...
export class DrivePad {
  ...
  constructor(
    ...
    private readonly nitro: HTMLButtonElement,
    private readonly controls: DriveControls,
    onInput: () => void,
  ) {
    ...
    ) => this.scope.listen(target, type, fn as EventListener, options);
    this.knob = root.querySelector<HTMLElement>(".stick-knob")!;
    for (const element of [root, nitro])
    ...
    for (const name of [
      ...
      on(nitro, name, (event) => {
        ...
        if (nitro.hasPointerCapture(event.pointerId))
          nitro.releasePointerCapture(event.pointerId);
      });
    }
    on(nitro, "pointerdown", (event) => {
      if (
        nitro.disabled ||
        (event.pointerType === "mouse" && event.button !== 0)
      )
      ...
      nitro.setPointerCapture(event.pointerId);
      onInput();
    });
    on(nitro, "keydown", (event) => {
      if (event.code !== "Space" && event.code !== "Enter") return;
      event.preventDefault();
      event.stopPropagation();
      if (!nitro.disabled && !event.repeat) {
        controls.pressAction("nitro-key", "turbo");
        onInput();
      }
    });
    on(nitro, "keyup", (event) => {
      if (event.code !== "Space" && event.code !== "Enter") return;
      event.preventDefault();
      event.stopPropagation();
      controls.releaseAction("nitro-key");
    });
    on(nitro, "focusout", () => controls.releaseAction("nitro-key"));
  }
  ...
  private releaseStick(): void {
    ...
    if (id !== null && this.root.hasPointerCapture(id))
  ...
  update(dt: number, boosting: boolean): void {
    ...
    while (remaining > 0) {
      ...
      this.velocityX +=
        ((this.x - this.visualX) * 420 - this.velocityX * 32) * step;
      this.velocityY +=
        ((this.y - this.visualY) * 420 - this.velocityY * 32) * step;
      this.visualX += this.velocityX * step;
      this.visualY += this.velocityY * step;
      remaining -= step;
    }
    ...
    this.nitro.setAttribute(
      "aria-pressed",
      String(this.controls.actionHeld("turbo")),
    );
  }
  setTurboAvailable(available: boolean): void {
    this.nitro.disabled = !available;
    this.nitro.title = available
      ? "Hold with forward throttle, or during cruise"
      : "Nitro is available on BuggY";
  }
  dispose(): void {
    this.clear();
    this.scope.dispose();
  ...
  clear(): void {
    ...
    for (const id of this.nitroPointers) {
      ...
      if (this.nitro.hasPointerCapture(id))
        this.nitro.releasePointerCapture(id);
    }
