...
export function validateBundle(value: unknown, id: WorldId): WorldBundle {
  ...
  if (
    ...
    !v.theme ||
    v.theme.length > 80 ||
    (v.kind !== undefined && (typeof v.kind !== "string" || !v.kind)) ||
    typeof v.title !== "string" ||
    !Array.isArray(v.elements) ||
    v.elements.length > 160
  )
    throw new Error("Invalid world bundle");
  ...
  for (const e of v.elements) {
    if (
      ...
      typeof e.id !== "string" ||
      ids.has(e.id) ||
      !["Button", "Card", "Panel", "Terminal", "Portal"].includes(e.kind) ||
      ![e.x, e.z, e.w, e.d, e.h].every(Number.isFinite) ||
      Math.abs(e.x) > 12000 ||
      Math.abs(e.z) > 12000 ||
      e.w < 2 ||
      e.d < 2 ||
      ...
      e.h > 12 ||
      (e.label !== undefined &&
        (typeof e.label !== "string" || e.label.length > 80)) ||
      (e.role && !["surface", "accent", "border", "text"].includes(e.role)) ||
      (e.state &&
        !["idle", "hover", "pressed", "selected", "disabled"].includes(e.state))
    )
      throw new Error("Invalid UI primitive");
    ids.add(e.id);
  }
  return v;
}
