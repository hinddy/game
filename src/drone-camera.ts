...
export class DroneCamera {
  ...
  recenter(): void {
    ...
    this.radius = Math.hypot(6.6, 3.9);
    this.returnDelay = 0;
    this.snap = true;
  }
  ...
  update(
    ...
    const angle =
      Math.atan2(this.heading.x, this.heading.z) + Math.PI + this.yaw;
    ...
    this.target
      .copy(position)
      .addScaledVector(this.heading, 1.5 * Math.max(0, Math.cos(this.yaw)));
    this.target.y += 0.85;
...
export class DroneGestures {
  ...
  constructor(
    ...
    private readonly drone: DroneCamera,
  ) {
    ...
    on(canvas, "pointermove", (event) => {
      ...
      if (this.previous) {
        ...
        if (next.distance > 8 && this.previous.distance > 8)
          drone.zoom(this.previous.distance / next.distance);
      }
      this.previous = next;
    });
    for (const name of [
